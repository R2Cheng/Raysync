...
def step_2(token,stime,etime):
    url = []
    response = getAllShareLink(token,stime,etime,type=1)
...
def step_3():
    ...
    createUrl(token,userId,shareUrlType=1,shareFiles=["/"])
    createUrl(token,userId,expireTime=1,shareUrlType=1,shareFiles=["/"])
    createUrl(token, userId, expireTime=3,shareUrlType=1,shareFiles=["/"])
    createUrl(token, userId, expireTime=7,shareUrlType=1,shareFiles=["/"])
    createUrl(token, userId, expireTime=30,shareUrlType=1,shareFiles=["/"])
    createUrl(token, userId, expireTime=-1,shareUrlType=1,shareFiles=["/"])
...
class TestShareLinkTwo():

    @allure.story("用例--查看分享下载链接列表--预期成功")
    @allure.description("该用例是针对业务场景的测试")
    @allure.title("查看分享下载链接列表-列表为空--预期成功")
    @pytest.mark.multiple
    # @pytest.mark.usefixtures("")#执行用例前可以进行前置操作(数据库操作,尽量不使用)
    def test_raysync_sharelink_two_01(self, testcase_data):
        ...
        logger.info("*************** 获取token ***************")
        token = step_1()
        logger.info("*************** 执行清空分享下载链接操作 ***************")
        step_2(token,stime,etime)
        logger.info("*************** 开始执行用例 ***************")
        result = getAllShareLink(token,stime,etime,type=1)
        ...
        logger.info("分享下载链接数量 ==>> 期望结果：{}， 实际结果：【 {} 】".format(totalCount, result.json().get("totalCount")))
        logger.info("*************** 结束执行用例 ***************")

    @allure.story("用例--查看分享下载链接列表--预期成功")
    @allure.description("该用例是针对业务场景的测试")
    @allure.title("查看分享下载链接列表-列表不为空，共6条链接（5条使用中（永久，1天，3天，7天，30天），默认参数（无需邮件通知），"
                  "1条过期，所有链接没有有效下载文件，只测试链接）,有效链接验证通过--预期成功")
    @pytest.mark.multiple
    # @pytest.mark.usefixtures("")#执行用例前可以进行前置操作(数据库操作,尽量不使用)
    def test_raysync_sharelink_two_02(self, testcase_data):
        stime = testcase_data["stime"]
        etime = testcase_data["etime"]
        ...
        logger.info("message ==>> 期望结果：{}， 实际结果：【 {} 】".format(except_msg, result.json().get("message")))
        logger.info("分享下载链接数量 ==>> 期望结果：{}， 实际结果：【 {} 】".format(totalCount, result.json().get("totalCount")))
        logger.info("分享下载链接使用中数量 ==>> 期望结果：{}， 实际结果：【 {} 】".format(useCount, useCountNum))
        logger.info("分享下载链接过期数量 ==>> 期望结果：{}， 实际结果：【 {} 】".format(overCount, overCountNum))
        logger.info("分享下载链接取消数量 ==>> 期望结果：{}， 实际结果：【 {} 】".format(cancelCount, cancelCountNum))
        logger.info("*************** 结束执行用例 ***************")


    @allure.story("用例--查看分享下载链接列表--预期成功")
    @allure.description("该用例是针对业务场景的测试")
    @allure.title("查看分享下载链接列表-列表不为空--使用中和过期链接取消邀请上传成功，并验证url不通过--预期成功")
    @pytest.mark.multiple
    # @pytest.mark.usefixtures("")#执行用例前可以进行前置操作(数据库操作,尽量不使用)
    def test_raysync_sharelink_two_03(self, testcase_data):
        stime = testcase_data["stime"]
        etime = testcase_data["etime"]
        useCount = testcase_data["useCount"]
        overCount = testcase_data["overCount"]
        ...
        logger.info("*************** 执行清空分享下载链接操作 ***************")
        step_2(token,stime,etime)
        logger.info("*************** 执行分享下载链接数据初始化操作 ***************")
        step_3()
        logger.info("*************** 开始执行用例 ***************")
        result_old_url = getAllShareLink(token,stime,etime,type=1)
        shareLink = []
        useCountNum = 0
        overCountNum = 0
        cancelCountNum = 0
        ...
        result_new_url = getAllShareLink(token,stime,etime,type=1)
        ...
        logger.info("分享下载链接数量 ==>> 期望结果：{}， 实际结果：【 {} 】".format(totalCount, result_new_url.json().get("totalCount")))
        logger.info("分享下载链接使用中数量 ==>> 期望结果：{}， 实际结果：【 {} 】".format(useCount, useCountNum))
        logger.info("分享下载链接过期数量 ==>> 期望结果：{}， 实际结果：【 {} 】".format(overCount, overCountNum))
        logger.info("分享下载链接取消数量 ==>> 期望结果：{}， 实际结果：【 {} 】".format(cancelCount, cancelCountNum))
        logger.info("*************** 结束执行用例 ***************")
    ...
    @allure.story("用例--查看分享下载链接列表--预期成功")
    @allure.description("该用例是针对业务场景的测试")
    @allure.title("查看分享下载链接列表-列表不为空--删除1个有效链接和1个过期链接--预期成功")
    @pytest.mark.multiple
    # @pytest.mark.usefixtures("")#执行用例前可以进行前置操作(数据库操作,尽量不使用)
    def test_raysync_sharelink_two_04(self, testcase_data):
        stime = testcase_data["stime"]
        etime = testcase_data["etime"]
        useCount = testcase_data["useCount"]
        overCount = testcase_data["overCount"]
...
if __name__ == '__main__':
    pytest.main(["-q", "-s", "test_05_raysync_sharelink_two.py"])